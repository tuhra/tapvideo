							...
							@foreach ($vdos as $vdo)
								@if(!in_array($vdo['privacy']['view'], $validation))
									 <div class="col-md-2 col-xs-3 marginBot-30">
						            	<center>
						            		<a href="{{ url('videos'.'/'.$vdo->video_id) }}?myid_auth_password=taptubemyid@tech2020" class="cat">
						            			@if(is_file(public_path('upload/video_thumbnails/' . $vdo->video_id. '.jpg')))
				                                	<img src="{{ asset('upload/video_thumbnails/'.$vdo->video_id.'.jpg') }}" class="img-responsive" style="width:100%;">
				                                @else
